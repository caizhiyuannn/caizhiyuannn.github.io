...
基本协议由header 和 content 部分组成（对比HTTP协议）。 header 和 content 部分通过 '\r\n' 分割开。

### Header Part 头/标题 部分

The header part consists of header fields. Each header field is comprised of a name and a value,
separated by ': ' (a colon and a space).
头部分包含头字段信息。每个头字段信息由name 和 value 组成，通过': ' 分隔开(一个冒号和空格).
...
考虑到最后一个头字段信息跟整个头字段信息都是以'\r\n'结束，并且必须至少有一个头字段信息，意味着在content部分开头会有两个'\r\n'。
...
Content-Length: 内容部分的长度，单位：字节。这个头信息是必须的。
Content-Type: 内容部分的mime类型，默认为application/vscode-jsonrpc; charset=utf-8

The header part is encoded using the 'ascii' encoding. This includes the '\r\n' separating the header and content part.
header部分使用ascii进行编码，包括分割header和content部分的'\r\n'
